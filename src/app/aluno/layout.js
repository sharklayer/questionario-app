...
const _navigation = [
  { name: 'Home', href: '/aluno/home', icon: HomeIcon, current: true },
  { name: 'Atividades', href: '/aluno/atividades', icon: ClipboardDocumentListIcon, current: false },
]
...
export default function Layout({ children }) {
  ...
  const { status, data: session } = useSession({
    required: true,
    onUnauthenticated() {
      router.replace('/auth/signin'); // Redireciona se não autenticado
    },
  });

  // Atualiza o estado de navegação com base no pathname
  useEffect(() => {
  ...
  // Redireciona administradores para a página inicial do admin
  useEffect(() => {
    if (status === "authenticated" && session?.user?.isAdmin === true) {
      router.replace('/admin/home'); // Redireciona admin para a página admin
    }
  }, [status, session, router]);

  // Enquanto redireciona, não renderiza nada para administradores
  if (status === "authenticated" && session?.user?.isAdmin === true) {
    return null; // Evita renderizar o layout enquanto redireciona admins
  }

  // Renderização para alunos autenticados
  ...
  return null; // Exibe nada enquanto redireciona
}